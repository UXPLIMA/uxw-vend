 ...
 * until we wire k-anonymity; the optional lookup lives in
 * `password-breach.ts`.
 *
 * Nothing in this file touches node, on purpose. The setup wizard has to
 * apply the same rules the setup API applies, in the browser, before it
 * lets the installer walk through four more steps and collect a 400 at the
 * end. Keeping the breach lookup in its own file is what makes that safe.
 */
