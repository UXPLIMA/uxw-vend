 ...
 *
 * A name is interpolated straight into `--blysis-color-<name>: <value>;`. One
...
describe("the style block an operator's colours become", () => {
    ...
    it("declares a colour under its own name", () => {
        expect(css({ primary: "#2563eb" })).toContain("--blysis-color-primary: #2563eb;");
    });
    ...
    it("keeps the good names when a bad one travels with them", () => {
        ...
        expect(out).toContain("--blysis-color-primary: #111111;");
